    ...
    {                                         
    	...
    	if (selectedRow != -1) 
    	{
    		...
    		if (found != null) 
    		{
    			...
    			if (!cancel) 
    			{
    				if (amount >= 0 && !currentBuyer.canAfford(amount)) 
    				{
    					JOptionPane.showMessageDialog(null, currentBuyer + " cannot afford $" + amount + "!", "Too much!", JOptionPane.ERROR_MESSAGE);
    				}
    				else if (amount < 0 && !currentSeller.canAfford(Math.abs(amount))) 
    				{
    					JOptionPane.showMessageDialog(null, currentSeller + " cannot afford $" + Math.abs(amount) + "!", "Too much!", JOptionPane.ERROR_MESSAGE);
    				}
    				else 
    				{
